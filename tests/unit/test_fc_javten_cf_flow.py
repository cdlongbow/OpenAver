...
class TestCfSiteOriginsParity:
    ...
    def test_per_site_behaviour_tables_cover_all_manual_only_sources(self):
        """transport 的三張 per-site 行為表也要 parity（pre-merge Stage 2 FINDING 3；
        `_SITE_FETCH_ORIGIN_PIN` 於 TASK-118a-T10/F-2 加入）。

        三張表都以 `.get(key, False)` fail-closed，所以漏列**不會拋錯、不會有任何測試轉紅**，
        ...
        for table_name in ("_SITE_AGE_GATE", "_SITE_NAV_RESET", "_SITE_FETCH_ORIGIN_PIN"):
            ...
            assert missing == [], (
                f"{table_name} 漏列 manual_only 來源 {missing} —— 三張表都是 fail-closed 的 "
