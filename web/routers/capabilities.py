...
_TOOLS: list[dict] = [
    ...
    },
    {
        ...
        "output_schema": {
            ...
            "blocked": "boolean — 僅 success=false 時出現。true=來源被擋／連不上（≠ 查無此片），稍後重試可能就有結果",
            "blocked_sources": "[string] — 被擋的來源 id（blocked=false 時為空陣列）",
        },
        "side_effect": True,
    ...
    },
    {
        ...
        "output_schema": {
            ...
            "reason": "string — hit|no_cover|not_found|blocked|error（blocked=來源被擋／連不上，≠ 查無此片，稍後重試可能就有結果）",
        },
        "side_effect": True,
        "confirmation_required": True,
        "idempotent": False,
    ...
    },
    {
        ...
        "output_schema": {
            ...
            "result_item": "{type: 'result-item', number, file_path, success, nfo_written, cover_written, source_used, reason, error?}（reason: hit|no_cover|not_found|blocked|error；blocked=來源被擋／連不上，≠ 查無此片，該片不會被記為「已嘗試」，稍後重試可能就有結果；唯讀來源片改道 output_dir 產生、成功項回 nfo_written/cover_written 及 reason（hit/no_cover，與非唯讀項同語意）、不再回 readonly 拒絕）",
