...
> 這一版修的是**開不起來**。如果你的 OpenAver 一直開得好好的，這一版對你沒有任何可見差異——沒有新功能，你的資料一個位元組都沒動。
...
- 伺服器背景程序如果自己出錯掛掉，完整的錯誤內容也會寫進 `debug.log`（但**不會**丟到畫面上——那對一般使用者沒有意義）。
...
測試數 7130 → 7166（pytest；npm test 未變動）。
