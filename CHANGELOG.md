...
本版只做一件事：**把「外部媒體管理器」設為「關閉」時，雲端／NAS 唯讀來源不再多產出兩張沒人看的圖**。對絕大多數使用者是無感的空間節省；畫面、播放、縮圖、對焦全部不變。
...
- **設定值不合法時一律當成「關閉」處理**：設定檔若被手動改壞（值打錯大小寫、留成空值、或殘留舊版本的寫法），過去圖片會被擋下、但 NFO 裡仍會寫入只有媒體管理器該看的欄位（`<lockdata>` 等），等於只擋了一半。現在這種值會統一收斂成「關閉」，圖片與 NFO 兩邊行為一致。合法的四個選項完全不受影響。
...
- 來源金絲雀 **7 通過／1 失敗（heyzo）**：與前兩版相同，已確認為站方把頁面 toggle 回純 JS 動態載入，屬外部網站行為、與本版改動無關。
