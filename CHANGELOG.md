...
- **命中封面飛入圖書館**：命中且封面可服務時，真封面從進度卡飛向側邊欄「瀏覽」入口（複用 spec-92 `playInboundFly`，bulk 變體：不懸停、飛行縮短成輸送帶感），落地 scale/glow + 側邊欄微光；進度卡上累計 badge（本次 run 補到資料片數，run 後淡出，不做持久紅點）。
...
### 測試
- 全套 pytest **5520 passed, 2 skipped**（unit + integration，排除 smoke／e2e，較 0.11.8 的 5498 +22：reason 映射邊界〔hit/no_cover 依「/thumb 可服務真相＝DB cover_path」分流、not_found/error 各分支〕+ 兩個互補 mutation-verified 回歸鎖〔擋「cover_written alone」+ 擋「退回磁碟真相」〕+ result-item 三站台 SSE reason 契約 + /thumb 時序鎖）＋ `ruff check .` 綠 ＋ `npm run lint`（eslint + stylelint）綠。
