...
本版是一次純內部收斂（feature/109）：把唯讀來源「單片刮削」與「批次補料」兩處各約 40 行、幾乎一模一樣的產出邏輯（決定要沿用本地資料還是重新刮削、實際寫檔、失敗時回什麼形狀）收斂成單一入口、兩邊共用。**對使用者完全無感、零行為變更**——這版不新增功能，純粹是把「同一條規則要記得改兩個地方」的維護債消掉。收斂過程中額外抓到兩條差點被放行的真 bug，一併修掉。

### Fixed
- **唯讀單片重刮：寫檔完成後若最終檢查失敗，舊封面縮圖不再卡住不消失**。以前若在封面已經寫入、但最後一步確認（例如當下資料庫被鎖、網路磁碟斷線）時出錯，介面會留著整理前的舊封面縮圖、且不會自己恢復正常，容易讓人誤以為整理沒生效。
- **批次補料：同樣的失敗情境下，封面對焦位置不再對不上**。以前若在寫檔完成後的最終檢查發生錯誤，新封面會沿用舊封面的手動對焦座標，導致自動裁切位置跟新封面對不上；本版改為此情境下會重新排定對焦。
...
- **Pre-merge 跨廠牌 review 計分卡**：codex 提 1（中等風險）、採納 1（批次補料的封面對焦排序問題，即上述已修的第二條回歸）；另一顆 Opus（high）提 1（中等風險）、採納 1（四處測試斷言因程式搬移後失去效力、變成沒在驗證任何東西的空殼）+ 3 條低風險不阻擋；grok 因本機環境缺少沙箱工具無法啟動，本輪未能執行、不列入計分。
...
- **發售日補日期**：已經抓到發售日的就直接顯示、不動它；只有沒抓到日期的無碼片（拖檔／選檔進來時）才會出現一個日曆，讓你點一下挑日期補上——只能用日曆點選，不能手打，所以不會有日期格式跑掉的問題。不確定就不選，留空即可（不要為了填滿而亂猜，假精確反而劣化資料）。
