...
> 這一版只影響 **javdb 這一個來源**，而且**畫面上沒有任何改動**。修的是一件長期查不出來的事：有些人的 javdb「永遠沒有結果」，但他用瀏覽器打得開。你的資料一個位元組都沒動。
...
### 已知限制
- **素人番號仍然查不到**（`259LUXU-1234`、`300MIUM-1077` 這一類）。這跟本版修的兩件事無關，是各站番號寫法不同造成的（javdb 把它記成 `LUXU-1234`，吃掉了前面那串數字），要另外處理。**變通做法還是老樣子**：用進階搜尋手動指定來源與番號。
- 「開著系統代理仍然連得到 javdb」這件事**需要真的裝了代理軟體的機器才驗得出來**；自動化測試涵蓋的是「代理設定有沒有被正確讀到、有沒有真的送出去」那一段。
- 這一版**沒有**改變 javdb 取得資料的方式，只改了連線與記錄。安裝路徑含中日韓文字時 javdb 可能連不上的既有限制**仍然存在**（變通做法：裝在純英文路徑）。

測試數 7204 → 7239（pytest，本版新增 35 支；npm test 1293，本版未新增前端測試）。
