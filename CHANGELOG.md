...
#### 🎯 女優頭像也能手動對焦
- 燈箱右上角加一顆對焦鈕，按下去會自動抓一次臉的位置，抓到後可以拖曳微調，✓ 存起來、✗ 取消不寫入。
- 系統會自動判斷這張照片是寬還是窄，只讓你往合理的方向拖（寬圖左右拖、窄圖上下拖），滑鼠移到可拖曳的框上也會提示拖曳方向。
- 就算這張照片抓不到臉（例如非正臉照、藝術照），框會停在置中位置，你還是可以手動拖到想要的地方存起來，不會卡住、也不會失敗給你看。
