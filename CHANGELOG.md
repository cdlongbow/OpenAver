...
- **以上這些只適用「輸入番號」的搜尋。** 用關鍵字（片名片段、女優名）搜尋時，javdb 走的仍然是舊的網頁做法——那些結果的封面**還是會有浮水印**，被 javdb 擋下來時**還是會沒有結果**。這是刻意的：關鍵字搜尋一次要抓十幾二十筆，全走新的那條會把它打壞，而它壞掉的話番號搜尋也跟著沒得用。
- 走**備援**那條時，「安裝路徑含中日韓文字」的限制仍然存在（變通做法：裝在純英文路徑）。
- 「開著系統代理仍然連得到 javdb」這件事**需要真的裝了代理軟體的機器才驗得出來**；自動化測試涵蓋的是「代理設定有沒有被正確讀到、有沒有真的送出去」那一段。

測試數 7204 → 7398（pytest，本版新增 194 支；npm test 1293，本版未新增前端測試）。
...
## 0.14.x 系列（早期，v0.14.0 ~ v0.14.4, 2026-08-19 ~ 2026-08-21）
