...
- 手機上因為 footer 只留得下名字，作品數與年齡會一起跟著下來，所以**同一個數字不會在一張卡上出現兩次**。
...
- 女優卡的資訊區只是顯示，**數值點不下去**（不像影片卡的片商／發售日可以點成篩選條件）。女優的年齡／身高／罩杯條件仍然從漏斗選單掛。

測試數 npm test 1056 → 1108（本次零 Python 改動，pytest 不變）。
