# Changelog Archive (v0.1.0 ~ v0.15.16)
...
- [0.16.1] 2026-09-17 — 燈箱女優名字後面顯示「發行時幾歲」＋「補資料」按鈕不再出現在按了必定失敗的片上＋沒封面沒番號時燈箱補上文字說明
- [0.16.0] 2026-09-16 — 瀏覽頁牆的展開／收起與進場動畫整體化（卡片滑開、資訊淡入、只動可見卡、整批固定時間預算由中心擴散）

### 0.15.x

- [0.15.16] 2026-09-15 — 四個「按鈕按了卻沒生效」的後端修復（片庫放共用資料夾最上層時整理 100% 失敗 ＋ 一鍵補完對只缺封面的片空轉 ＋ 搜尋頁整理寫出的 NFO 掉簡介與評分）＋ 新片入庫的來源選擇語意收斂成正典
- [0.15.15] 2026-09-11 — 新手引導第 3 步改成直接畫出等一下會出現的卡片 ＋ 搜尋頁空狀態一行講一顆按鈕並即時算出檔名範例 ＋ 五個頁面工具列／頁首／內容左緣對齊同一條線（外觀材質零改動）
- [0.15.14] 2026-09-10 — 檔名就是番號的片「一鍵補完」不再卡在番號當標題 ＋ 回到瀏覽頁不再重傳整個片庫（ETag/304）＋ Windows 一鍵安裝視窗點一下不再凍住 ＋ 兩支安裝程式四語 ＋ wiki 換圖白名單補新網域
- [0.15.13] 2026-09-05 — 「定時整理」：每 12 小時自動整理最愛資料夾、手動操作時自動避讓 ＋ 查無結果／已整理過不再重問全部來源 ＋ 側欄通知上限 10→50 則且重開機不清空
- [0.15.12] 2026-09-04 — 唯讀來源比照一般掃描：解不出番號也會建卡（逃生口不再消失）＋ 番號解析／片商對照改用同一張表 ＋ 自訂標籤與補完 NFO 改寫輸出夾、不再誤寫來源資料夾
- [0.15.11] 2026-09-03 — 封面牆先偵測連不到的來源位置：底部狀態列直接列出無法存取的位置、斷線時不再逐張等逾時 ＋ 正常 10 分鐘／異常 1 分鐘的重新確認頻率
- [0.15.10] 2026-09-03 — 書籤牆與書籤燈箱補齊動效（淡入淡出、封面飛入飛出、移除後滑動補位）＋ 加入滿 14／30 天的天數提示膠囊 ＋ 燈箱內移除並支援手機滑動換片
- [0.15.9] 2026-09-02 — 書籤自動對帳（入庫即移除、後端擋重複加入）＋ 半截番號也能切格牆／大卡 ＋ 書籤清單開啟成本脫離片庫規模
- [0.15.8] 2026-09-02 — 書籤：想看還沒入手的片先收起來（獨立牆與燈箱、封面當下存本機、已入手自動沉底並可一鍵清理）
- [0.15.7] 2026-09-01 — 「算不算番號」的判斷從八處各寫各的統一成同一張表（手動輸入不再被改壞、前面帶數字的番號走精準搜尋、FC2 番號統一 `FC2-<數字>`）
- [0.15.6] 2026-08-31 — 瀏覽頁手感修復五項（拖曳取消後整頁點不動、女優大卡對焦拖了沒用、兩牆互切跳到最底、切回瀏覽頁閃一下）＋ 女優大卡展開後補滿空白
- [0.15.5] 2026-08-30 — 一條永遠通過的架構守衛改用 import-linter 重寫 ＋ 刪 1,144 行零覆蓋測試 ＋ 六個「剪斷也沒人發現」的接線各補一道檢查（零可見改動）
- [0.15.4] 2026-08-29 — AI 助理可提交審過的資料直接寫進 NFO／收藏庫（不再只能叫 OpenAver 重刮）＋ 四道保護
- [0.15.3] 2026-08-29 — DMM 番號對照表改成隨版本出貨（164 條）＋ 停用本機自學值（53 條裡 21 條是錯的）＋ 兩條補償查詢路徑
- [0.15.2] 2026-08-28 — 瀏覽頁動畫成本（1199ms → 93ms）＋ 指示燈停轉 ＋ 呈現切換改一顆鈕、表格／清單改設定內開啟
- [0.15.1] 2026-08-28 — javdb 改走官方 App 資料通道（網頁解析留備援）＋ 補齊五個欄位＋封面去浮水印
- [0.15.0] 2026-08-27 — 四個全螢幕層補上鍵盤焦點鎖 ＋ 前端狀態撞名守衛／toast／行內編輯／說明浮層四項內部收斂

### 0.14.x

- [0.14.10] 2026-08-25 — 開著代理軟體時不再一啟動就崩潰 ＋ metatube 來源逾時一次不再永久消失
- [0.14.9] 2026-08-25 — 清除鈕／搜尋狀態不再跨分頁互相干擾 ＋ 搜尋列在不同桌機寬度下對齊
- [0.14.8] 2026-08-24 — 瀏覽器下四個資料夾入口接上網頁版選擇視窗 ＋ 拖放不再靜默失敗 ＋ `docs/linux-server.md` Linux 伺服器部署文件
- [0.14.7] 2026-08-24 — 補劇照不再覆蓋既有圖
- [0.14.6] 2026-08-23 — metatube 來源劇照代理修復（白名單漏收冷門圖床）＋ 抓不到原址自動改走 metatube（存檔仍優先原址）
- [0.14.5] 2026-08-22 — 第三方套件版本／授權紀錄補齊 ＋ `docs/THIRD_PARTY.md` ＋ 換套件忘記更新的守衛（零可見改動）
- [0.14.4] 2026-08-21 — 發售日可點成條件（含區間）＋ 女優牆眼睛鈕與可點數字 ＋ 燈箱換片箭頭對齊封面
- [0.14.3] 2026-08-21 — 一鍵精選（星標 ＋「只看精選」條件 ＋ AI 批次端點）＋ 冷門女優從片庫收藏
- [0.14.2] 2026-08-20 — 分集片合併呈現 ＋ 批次整理撞名標記
- [0.14.1] 2026-08-20 — 檔名版本後綴自動變成標籤（中字／無碼破解／無碼流出／4K／VR）+ 封面屬性標籤 opt-in
- [0.14.0] 2026-08-19 — 燈箱／瀏覽器播放失敗就地提示 + Windows WebView2 偵測與安裝流程修復 + 罩杯排序支援 L 以上

### 0.13.x

- [0.13.14] 2026-08-15 — 桌面影片牆新增「直式海報」卡型選項（與完整封面並列，純顯示裁切不動封面原檔）
- [0.13.13] 2026-08-14 — FC2 備援來源 FC2-javten 上線，補官方站已下架片（僅 Windows 桌面版、手動選用）
- [0.13.12] 2026-08-14 — FC2 搜尋修復（改打官方站）+ 劇照副檔名支援對齊 Jellyfin + 側邊欄 logo 破圖修復
- [0.13.11] 2026-08-12 — 女優牆新增「從片庫加入女優」面板（依片數排序、一鍵收藏）
- [0.13.10] 2026-08-11 — 女優牆年齡/身高/罩杯可點擊篩選，並可改為以下/以上/自訂區間
- [0.13.9] 2026-08-10 — 瀏覽頁篩選改為可疊加條件標籤（女優/標籤/片商/導演/系列/廠牌，取交集）
- [0.13.8] 2026-08-09 — AI agent 專屬連線 token（需先開密碼保護）+ API 金鑰/metatube token 不再完整送進瀏覽器
- [0.13.7] 2026-08-09 — 區網連線密碼保護（4 位英數 + 偽裝頁 + 全域重試鎖定）+ 公網暴露警告 + CSRF/計次兩道補強
- [0.13.6] 2026-08-07 — metatube「換來源」預覽破圖修復（preview_cover_url 與 cover_url 分離）+ metatube 憑證三處外流封閉 + 圖片來源放行政策收斂為宣告式 registry
- [0.13.5] 2026-08-06 — 第三方 `.nfo` 的演員／片商／發行日／分類標籤被誤判缺漏後遭刮削覆寫（四種寫法讀不到）修復 + 空白欄位與全空白標籤的五條靜默 bug + NFO 解析／sidecar stat／原子寫三塊決策所有權收斂
- [0.13.4] 2026-08-05 — 媒體伺服器模式封面正典位置翻面（不再產同名封面、`-fanart` 升格）修好 Emby/Jellyfin 封面牆橫式圖塞直式格 + 唯讀來源三條同檔覆寫 bug
- [0.13.3] 2026-08-04 — Jellyfin 補圖裁毀「只有 -poster」的片封面原檔（不可還原）修復 + -fanart 片誤判缺 NFO 修復 + 封面正典位置收斂單一決策點（純代換）
- [0.13.2] 2026-08-03 — 唯讀來源「關閉」模式不再多產 -poster/-fanart（省空間）+ HEYZO 站方改版搜尋修復
- [0.13.1] 2026-08-02 — 安全修補（Pillow/python-multipart 消 17 CVE）+ 路徑逃逸最終防線 + 檢查更新按鈕三道防護
- [0.13.0] 2026-08-01 — 唯讀來源單片/批次產出邏輯收斂單一入口（純內部，零行為變更）
...
- **這一版新增的文字只有繁體中文**：介面新增的畫面暫時只有中文，英文／日文／韓文會先沿用中文顯示，之後統一補上其他語言。（已於 0.15.15 補齊。）
...
> 這一版加的是**書籤**：看到想看、但還沒入手的片，可以先收起來。搜尋框左邊多了「搜尋結果 ／ 書籤」的切換，書籤有自己的一面牆和燈箱；等你哪天把片掃描入庫了，那筆書籤會自動沉到後面並標上記號，也可以一鍵把已經入手的整批清掉。封面在你按下書籤的當下就存到本機，所以原站掛掉、圖被刪、甚至沒連網，書籤牆都還是完整的。**你的影片檔與既有資料庫一個位元組都沒動**（書籤存在一張新的表裡）。

### Added

#### 🔖 把想看的片先收起來

- 搜尋結果的卡片右上角現在是**三選一**：已經在片庫裡的顯示播放鍵（同一番號有多個檔案時多一顆資料夾鍵），不在片庫裡的顯示書籤鍵，已經收藏的顯示移除鍵。同一組三態在格牆、燈箱、單片大卡三個地方一致。
- 搜尋框左側新增「搜尋結果 ／ 書籤」切換，書籤那顆帶著目前的數量。

#### 🖼 書籤有自己的一面牆和燈箱

- 切到書籤會看到整面書籤格牆，點任一張開燈箱看大圖：左右箭頭、鍵盤方向鍵、滑鼠滾輪都能換片，Esc 關閉。有劇照的片可以再點進劇照集。
- **封面在你按下書籤的當下就存到本機**（轉成 webp）。之後不管原站掛掉、圖被刪、還是你根本沒連網，書籤牆都不會變成一片破圖。

#### 📥 已經入手的會自己沉底

- 每次打開書籤清單都會跟片庫對一次帳：已經掃描入庫的書籤沉到最後面並標上「已入手」角標，讓還沒入手的留在前面。
- 上方多一顆「清理已入手」，一次把那些已經入手的書籤整批移除（**只動書籤，片庫與影片檔完全不碰**）。沒有已入手的書籤時這顆按鈕不會出現。

### 已知限制

- **書籤燈箱在手機／平板上還不能用手指滑動換片**，請用左右箭頭。（搜尋燈箱、劇照集、單片大卡三處本來就支援滑動，書籤燈箱這一版還沒接上。）
- **0.15.7 之前掃描入庫的 FC2 影片，書籤可能不會被標成「已入手」。** 番號格式在 0.15.7 統一成 `FC2-<數字>` 但舊列刻意不遷移，比對時就對不上。重新掃描那些片即可。

測試數 pytest 7988、npm test 1431。

## [0.15.7] - 2026-09-01

> 這一版整理的是**「一串字到底算不算番號」這個判斷**。它過去散在八個地方各寫各的，答案彼此不一致——於是同一個番號在搜尋框可以，在檔案清單就被改壞；在這個畫面查得到，在那個清單就漏掉。現在它們問的是同一張表。你的影片檔與資料庫一個位元組都沒動。

### Fixed
#### ✍ 在檔案清單手動輸入的番號，不會再被改壞

- 搜尋頁的檔案清單可以手動填番號。過去你打完之後它會**自己「修正」格式**，而修法是錯的：
  `n0762` 變成 `N-0762`、`200GANA-3360` 變成 `GANA-3360`、`FC2PPV-4943690` 變成 `PPV-4943690`。
- 這三種都是**真實存在的番號寫法**，被改掉之後當然查不到，而你看到的是「我明明打對了」。
- 現在你打什麼就送什麼（只去掉頭尾空白）。**格式判斷交給後端做**，那裡才有完整的規則。

#### 🔍 前面帶數字的番號，現在會走精準搜尋

- `200GANA-3360`、`259LUXU-001`、`T28-103`、`7IPZ-154`、`FC2-PPV-1234567` 這類**前面帶數字或混合前綴**的番號，
  過去不被當成「完整番號」——於是搜尋框把它們當成**女優名字**去查，結果不準或查不到。
- 現在它們會直接走精準搜尋。
- 同一個判斷也用在「整理 → 缺 NFO」那份清單上：**過去這類番號的片會從清單裡漏掉**，現在收得到
  （所以升級後那份清單可能會變長，那不是壞掉，是本來就該在裡面）。

#### 📋 把畫面上的字連包裝一起貼進搜尋框，**一般番號**現在也查得到

- 從封面複製 `[ABC-123]`、`【ABC-123】`、`ABC-123.mp4` 這類連同包裝符號、副檔名的字串貼進搜尋框，
  過去會被當成女優名字去查，查無結果。
- 現在會自動辨識出裡面的番號再查——手動輸入框（見上）與主搜尋框都適用。
- **例外：前面帶數字的番號連著檔名一起貼，會漏掉前面那組數字。**
  例如貼 `200GANA-3360.mp4` 或 `[200GANA-3360]`，實際會去查 `GANA-3360`——那是查不到的。
  **直接貼番號本身（`200GANA-3360`）就沒有這個問題**，上面那條就是在講這個。
  （這類番號的前綴到底算不算番號的一部分，牽涉到掃描入庫與資料庫既有資料，留待之後一次處理。）

### Changed
#### 🏷 FC2 番號統一成一種寫法

- FC2 的番號在檔名裡有七種寫法（`FC2PPV-1234567`、`FC2-PPV-1234567`、`FC2_PPV_1234567`…），
  過去掃描入庫時會照抄，於是同一批片在資料庫裡分裂成兩種形式。
- 現在一律存成 `FC2-1234567`，**與 FC2 來源本來就回傳的形式一致**。
  好處是「找相似的片」會把它們算成同一群（過去被拆成兩群）。
- **這一版之前已經入庫的舊資料不會自動改**——那些片要重新刮削一次才會併回同一群。

#### 🔎 用關鍵字在 javdb 搜尋時會多花一點時間

- javdb 的關鍵字搜尋現在會為候選結果多做最多 20 次查詢，以確認番號。慢一點，但比較不會漏。
- 用番號搜尋不受影響。

### Added
#### ✏ 拖進來的檔案查不到時，現在可以就地改番號

- 把檔案拖進來、底下出現**檔案列表**之後，如果**檔名的番號解析成功、但所有來源都查不到**
  （例如番號本身就打錯了），過去那個畫面是**死路**：沒有可以改番號的地方，
  也沒有進階搜尋的入口，只能把檔案移除重來。
- 現在那一列會出現 ✏️ 按鈕，點下去輸入正確的番號就會重新查。
  （檔名完全解析不出番號時本來就有這顆按鈕，這次是把「查不到」也納進來。）

#### 🧭 查不到的時候，畫面上多一條路

- 查無結果時，過去只有一句「請檢查番號格式或稍後再試」，然後就沒了。
- 現在下面多一顆「**使用番號進階搜尋**」，點下去會帶著你剛剛查的那個番號打開進階搜尋，
  可以自己挑來源（DMM／JavBus／JavDB／D2Pass／AVSOX／FC2…）再試一次。
- **底下有檔案列表的那種畫面還沒有這顆按鈕**——那邊請用上面那條的 ✏️ 改番號重查。

## [0.15.6] - 2026-08-31

> 這一版全是**瀏覽頁的手感修復**：三個「明明看得到、按了卻沒反應」的地方，兩個「畫面自己跳掉」的地方，加上左上角那張女優大卡不再空一大塊。你的影片檔與資料庫一個位元組都沒動。

### Fixed
#### 🖱 拖曳資料夾進掃描頁，取消之後整頁點不動

- 從檔案總管把資料夾拖進掃描頁，如果**中途拖到視窗外才放開**（或按 Esc 取消），那層「放開以加入」的覆蓋層**不會消失**——而它蓋住整頁，底下的按鈕全部點不到，只能重新整理。
- 原因：覆蓋層靠「進去幾次、出來幾次」的計數在開關，而從視窗外拖進來的檔案**根本不會發出結束訊號**，計數永遠歸不了零。
- 改成「只要滑鼠還在拖就每隔一下報一次平安」，停止報平安就自動收起來。**沒有計數就不會數錯。**

#### 🖼 女優大卡的對焦，拖了等於沒拖

- 女優照片太寬時可以拖曳調整要露出哪一段，調完存起來——女優牆吃這個座標，**但瀏覽頁左上角那張大卡從來不讀它**，永遠置中。對焦按鈕會出現、也拖得動、也真的存進去了，就是畫面不動。
- 另外**有別名的女優**還多一個問題：存好之後要重新整理才會變。
- 兩個都修了。存完當下就看得到。

#### ↕ 女優牆與影片牆互切，畫面跳到最底

- 在女優牆按「搜尋相關影片」，只要**當時不在最頂端**，切過去後畫面一律落在影片牆的**最底部**，得自己捲回去才看得到第一排。反方向（影片牆切回女優牆）同樣會跳。
- 兩個方向現在都回到頂端。

#### 🎬 從別頁切回瀏覽頁，整牆會閃一下

- 帶著女優大卡的狀態切到設定／掃描／搜尋，再切回瀏覽頁：畫面**先畫出沒有大卡的影片牆，接著整牆閃一下**，大卡才補到左上角、其他卡片跟著往後推一格。
- 現在兩者同一幀出現，不再有那一閃。

### Added
#### 🏷 女優大卡展開後不再空一大塊

- 展開資訊時（右上角眼睛圖示），旁邊的影片卡會長出四五行詳細資料，**女優大卡卻沒有任何對應內容**，於是它旁邊空出一塊與其他卡等高、什麼都沒有的區域。
- 這一版補了兩層：先是年齡／身高／罩杯／三圍一行（比照女優牆，**純顯示不可點**），再用標籤／出身地／事務所排成同一種藥丸形狀把剩下的空間填掉，塞滿為止、超出的不顯示。
- **手動加入、沒有這些資料的女優仍會留白**——那是資料本身沒有，不是畫面壞了。

## [0.15.5] - 2026-08-30

> **這一版對你沒有任何差別**——沒有新功能、沒有畫面改動、沒有行為改動。它整理的是我們自己的測試與守衛：把一條壞掉的檢查換成真的會擋的、刪掉一批什麼都沒在守的測試，並為六個「改壞了也沒人會發現」的地方補上真的會擋的檢查。你的影片檔與資料庫一個位元組都沒動。

### Fixed
#### 🛡 一條「一直是綠燈、但其實從來沒在檢查」的守衛

- 專案裡有一條規則：核心程式不可以去用 Windows 桌面版專屬的程式碼。它被寫成一個檢查，而那個檢查**寫錯了一個參數，所以永遠不會失敗**——不管有沒有違規都顯示通過。
- 這件事會怎麼咬到人：**照 `docs/linux-server.md` 把 OpenAver 裝在 NAS／Linux 伺服器上的人**。那份文件教的安裝方式會刻意排除桌面版才需要的套件；一旦核心程式誤用了桌面專屬的程式碼，**伺服器會直接起不來**。而開發機和 CI 都裝了那些套件，兩邊都是綠的——沒有人會事先知道。
- 現在改用真正做得到這件事的工具（`import-linter`）來檢查，並且實測驗證過「故意違規時它真的會擋下來」。

### Removed
- **刪掉 1,144 行沒有在保護任何東西的測試**（相似星座動畫的六支測試 ＋ 上面那兩條永遠通過的檢查）。它們的覆蓋率是 0 → 0：那六支測的是一份用 Python 抄寫的複本，不是實際在跑的程式碼。刪除位置留了註解，下次真的要改那塊時會看到。

### Added
#### 🔒 六個「改壞了也沒人會發現」的地方，現在改壞會被擋下來

- 我們抽樣量了一件事：**把畫面上某個功能的接線剪斷，我們的自動檢查會不會發現？** 抽了 13 個地方，**有 6 個剪斷之後三層檢查全部照樣顯示通過**。
- 那 6 個地方剪斷之後，你會遇到的分別是：瀏覽頁的搜尋框**打字完全不篩選**、掃描頁**拖放資料夾整個失效**（看起來還可以拖，放開卻沒事）、設定頁的命名格式**貼上文字沒有反應**、搜尋頁封面載不出來時**破圖救援不啟動**、女優卡與女優燈箱裡不該可以點的欄位**同時出現純文字和一個可點連結**（點下去是空的）。
- 這一版為這 6 個地方各補了一道檢查。**驗證方式是把每一條真的剪斷一次、確認檢查真的會擋**——而不是「寫完看起來對」（上一段那條壞掉的守衛就是這樣混過去的）。
- **這對你這一版沒有任何差別**：那 6 個功能現在是好的，一直都是好的。改變的是「以後有人不小心把它們弄壞時，會不會在出貨前被攔下來」。

### 已知限制
- 這一版沒有為相似星座補上真正的測試（那塊動畫不會沒事被修改；真的要改的時候本來就得補）。
- 上面那 6 個地方是**抽樣**抽到的，不代表其他地方都有守住。另外三個頁面（搜尋、掃描、設定）除了這次補的之外，其餘接線仍然沒有這類檢查——留給下次抽樣或真的有人踩到時再補。

測試數 7591 → 7549（pytest，本版刪除 42 支、未新增；npm test 維持 1330——本版沒有前端行為改動。新補的 6 道檢查走 lint 不走測試，所以測試數不變）。

## [0.15.4] - 2026-08-29

> 這一版只影響**你透過 AI 助理操作 OpenAver** 的時候。畫面上沒有新按鈕，你自己動手的操作一個位元組都沒變——沒在用 AI 助理的人，這一版對你沒有任何差別。你的影片檔與資料庫也一樣沒動。

### Added
#### 🤖 AI 助理可以「改好再交回來」，不再只能叫 OpenAver 重刮一次

- 過去 AI 助理能做的只有「叫 OpenAver 去某個站重抓」——**站方給什麼就是什麼**。它明明看得到八個來源的搜尋結果，卻沒有辦法把「封面用 A 站的、標題用 B 站的順便翻成中文、女優名單用 C 站的」這個它已經做好的判斷寫回去。
- 現在它可以了：AI 自己組一份審過的資料一次交回來，OpenAver 直接寫進 NFO 與收藏庫，**中間不再多打一次外站**（也就不會又被站方擋下、或又拿回你不想要的那一版）。
- 你會這樣用它：跟 AI 說「這幾部片的標題幫我翻成中文、片商欄位用 javdb 的、封面不要動」，它查完、挑完、翻完，一次交回來。

#### 🛡 為了讓上面那件事不會出事，同時加了四道保護

- **番號對不上就擋下來。** AI 交回來的番號跟收藏庫裡那部片本來的番號不一樣時直接拒絕，不會把別部片的資料蓋到你這部片上。真的要改號，得明確講出來才行。
- **只改文字時預設不動封面。** 改標題、女優、標籤的時候，你已經對好焦的封面、已經放好的海報與劇照都不會被重新產生一次。要換封面必須明確要求。
- **AI 沒提到的欄位不會被清空。** 簡介、評分、來源網址這三欄 AI 讀不到現在的值（查詢功能不會回這三個），所以它沒帶的時候沿用你 NFO 裡原本的值，不會因為「它沒寫」就被抹掉。
- **內部保留字不給冒充。** 交回來的來源欄位不接受 `db` 與 `nfo` 這兩個 OpenAver 自己在用的值——用了會變成「NFO 改了、收藏庫沒改」，而且**回應還是顯示成功**，你完全看不出來。

### Changed
- **指定來源時必須搭配精確番號搜尋。** 過去在關鍵字搜尋裡指定「只用 javdb」既不會有效果、也不會有任何提示（關鍵字那條路本來就不吃來源指定）；現在會直接說明這個組合不成立。只影響 AI 助理與直接呼叫 API 的人，**畫面上的搜尋列不受影響**。
- **AI 助理讀到的片長單位標示修正**——那一欄一直是分鐘，說明文字卻寫「秒」。

### 已知限制
- 這一版新增的能力**只開給 AI 助理**，畫面上沒有對應的按鈕。
- 簡介與評分可以寫進去，但**任何查詢功能都不會把它們回顯給 AI**，所以 AI 沒辦法「先讀現在的值、再決定要不要改」，只能整個換掉或完全不動。
- 評分是 0–5 刻度（NFO 裡存的是 0–10，OpenAver 自動換算）。送一個超出刻度的數字不會被擋——那是 AI 沒看說明，不是格式錯。
- AI 如果交回一份**全空**的資料，OpenAver 會照單全收。判斷資料對不對是 AI 的責任，不是 OpenAver 的。
- ⚠️ **「唯讀來源」的片這一版還不開放給 AI 改。** 後端收得下，但說明書上刻意沒有告訴 AI 怎麼觸發——因為那條路還有兩個沒補完的洞（改標題會讓輸出夾裡的封面對不上新檔名、以及簡介與評分會被清空）。與其開一個會出事的門，不如先關著，下一版補完再開。**你現在用唯讀來源的方式完全沒變。**

測試數 7485 → 7591（pytest，本版新增 106 支；npm test 維持 1330——本版是純後端改動，前端零檔案變動）。
...
### Fixed
- **你自己放的劇照不會再被蓋掉。** 按「補劇照」時，`extrafanart/` 資料夾裡已經存在的圖一律跳過、只補缺的那幾張——不管那張是你手動放的、Jellyfin 留下的，還是上次抓好的。過去是無條件重新下載覆蓋，原圖回不來。單片補完、批次補完、補劇照三個入口一體適用。

### 已知限制
- 下載到一半斷線留下的**壞圖**（不是 0 位元組、但打不開）也算「已經存在」，會被永遠跳過。要重抓請自己把那片的 `extrafanart/` 整個資料夾刪掉再按一次。
- 唯讀來源產出的劇照**不在此列**——那是 OpenAver 自己輸出夾裡的副本，設計上就可以重產。

其餘改動全是開發流程與測試隔離的內部整理（測試不再碰到真實片庫、開發模式改語言檔會自動重載、design-system 頁兩個內部樣式 bug），**使用者沒有任何可見差異**。

測試數 7048 → 7105（pytest；npm test 1189 不變）。
...
**桌面的封面牆，現在可以只看封面的正面。** JAV 橫式封面的「正面」在右半邊，左半邊是背面拼貼——手機版一直是只露右半的直立卡，桌面版卻從來沒有這個選項。本版把那個選擇權交給桌面：模式選單多兩條，**完整封面**（現況、預設）與**直式海報**。不碰影片檔、不碰 NFO、不碰封面原檔——直式海報是同一張封面在畫面上的裁切，不是另一個檔案。

### Added
#### 🖼️ 桌面影片牆的卡型選擇

- **在哪裡選**：工具列既有的模式選單（原本三條）在桌面寬度展開為四條：完整封面／直式海報／詳細／文字。**不新增第五顆按鈕**。
- **選了會記住**：跟排序、頁碼一樣。切到別的頁面再回來、關掉程式隔天再開，都還是你選的那種。
- **卡變窄了所以一列放得更多**：2100px 以上一列 8 張、1850–2099 是 7 張、1600–1849 是 6 張、1350–1599 是 5 張、1100–1349 是 4 張、900–1099 是 3 張。（完整封面在 1600px 以上一律 5 張。）
- **`A` 鍵多一段**：桌面按 `A` 依序是 完整封面 → 直式海報 → 文字 → 詳細 →（回到完整封面）。手機寬度維持三段。
- **切換是原地變形**：同一批封面留在畫面上，比例與位置連續變化，不是整面淡出再淡入。系統開了「減少動態效果」就直接到位。切換不重新載入資料、不改頁碼、不動捲動位置。
- **直式卡的常駐文字只留番號**：女優名收起來讓封面當主角。滑鼠停留仍顯示片名，眼睛鈕（`S`）展開的完整資訊一欄不減。
- **人臉被切到時可以自己拖回來**：桌面切到直式海報後，燈箱的焦點編輯鈕會出現（以前只有手機寬度才有）。拖到對的位置按 ✓ 存起來，封面牆上那張卡立刻跟著變。

### Changed
- **手機寬度（≤899px）一行不改。** 該寬度本來就強制直式，給一個選了沒用的選項比不給更糟。你在桌面選的那一種，把視窗拉窄再拉回來時會原封不動回來。
- **首次使用的人看到的畫面與上一版逐像素相同**——預設是完整封面，沒選過就等於沒有任何改變。

### 已知限制
- **這次新增的介面文字只有繁體中文**（「完整封面」「直式海報」）。簡中／英文／日文會回退顯示，下次 milestone 統一補。
- **方形／無碼封面在直式大卡上會被切掉左邊約三成。** 那些片會跑臉部偵測，偵測到就對準人臉；**偵測不到才退回右裁**。逃生口就是上面那顆焦點編輯鈕。
- **900–1099px 這一段沒有密度補償**：兩種卡型都是 3 欄，直式的列高比較高，所以一屏看到的片會變少。這是「桌面視窗被縮到一半」的寬度，要密度切回完整封面即可。
- **1920×1080 的螢幕，直式第一屏會少看到一張**（14 vs 15）。每垂直像素的密度其實是直式略高（+3.4%），差別出在「完整封面剛好塞得下第三列、直式差一點」。捲一下就有。
- **切換卡型後不會把切換前看到的那張片捲回視野**。直式與橫式的列高差約 1.5 倍，同一個捲動位置對應到的是不同的片。
- **側欄展開且視窗剛好 1100px 時，卡片比預期窄約 4px**（179 而非 183）。這是既有女優牆階梯逐像素相同的性質（本版沿用它），不是本版引入。

### 測試
- 全套 pytest **6728 passed, 1 skipped**；`npm test` **825**（上一版 774，本版新增 51 支）。`npm run lint`（含 `static_guard_lint` 1123 條、`css-guard` 49 條）、`ruff check .`、`lint-imports`、函式規模閘全綠，四條架構閘零新增豁免。零後端 diff，故未跑來源金絲雀（T3 儀式強度）。
- **七個 task 全綠、825 支測試全綠、兩支獨立 review PASS，而功能是死的。** plan 的改動檔案清單漏了 template 上那一行把卡型接到 CSS 的綁定，於是使用者點「直式海報」畫面完全沒反應——**它死在 task 與 task 的接縫上，不在任何一個 task 裡面**（兩端都有測試，中間那條線不屬於任何一個 task 的驗收條件）。抓到它的不是任何守衛，是收尾時把程式跑起來用真滑鼠點一次。已補上對應的機械守衛。
- **真機 CDP 驗收**（全新瀏覽器 profile ＋ 真實片庫 2089 部）：六段欄數階梯 12/12 全中且邊界逐格精確；899/900 三者同步翻面；切換動畫全程卡片透明度最小值為 1（確認不是淡出淡入）、容器高度未塌陷、連點 5 次後無殘留樣式；`A` 鍵四段、女優牆吃 `A` 零變化、焦點鈕三態全部成立；**US-5 完整流程**（遮罩幾何從來沒有在桌面寬度執行過）真滑鼠拖 100px → 遮罩位移逐像素相符、按 ✓ 後牆上那張卡的裁切位置真的跟著變，測完已還原原值。
- 守衛總量三桶棘輪 **11,810 行**（main 為 11,785，+25）：`static_guard_lint.mjs` +6（1 條新規則，守「卡型必須真的接到牆上」）、`css-guard.mjs` +19（**放寬**既有規則以容納卡型變體，規則數維持 49）、`test_frontend_lint.py` ±0。
...
- **驗證視窗萬一沒能啟動，那條來源會單獨灰掉。** 兩條需要驗證的來源（JavLibrary、FC2-javten）各自有一個獨立的驗證視窗。若其中一個因故沒建立成功，只有那一條會灰化並提示「請重新啟動 OpenAver」——**另一條不受影響、照常可用**。重開程式即可恢復。
...
## 0.13.x 早期 (v0.13.0 ~ v0.13.7, 2026-08-01 ~ 2026-08-09)

- **v0.13.7**：區網模式可設 4 位密碼（區分大小寫）；擋下的人看到的**不是登入畫面而是一頁「這網址沒東西」的偽裝頁**——登入畫面等於告訴掃到你 IP 的人這裡值得攻擊；輸過一次那台裝置永久記住，改密碼或關閉保護則一律全撤；連錯 5 次鎖 1 分鐘、逐級拉長至 1 小時且**不分裝置**（4 位密碼的保護力來自這裡不是長度）；另加公網暴露警告（**只抓得到「這台電腦自己有公網位址」，抓不到路由器上的 port forward**）。
